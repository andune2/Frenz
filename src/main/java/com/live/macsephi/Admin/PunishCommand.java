  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
  {
    if ((sender instanceof Player)) {
      ...
      if (this.me.hasPermission(player, "MobEffects.punish")) {
        if ((args.length == 1) && (this.me.getServer().getPlayer(args[0]) != null)) {
          ...
          if (player != target) {
            ...
            for (Player p : this.me.getServer().getOnlinePlayers()) {
              if (p == player) {
                player.sendMessage(ChatColor.DARK_RED + "You have inflicted punishment upon " + target.getName() + "!");
              }
              else if (p == target) {
                target.sendMessage(ChatColor.DARK_RED + player.getName() + " has inflicted punishment upon you!");
              }
              else
                p.sendMessage(ChatColor.DARK_RED + player.getName() + " has inflicted punishment upon " + target.getName() + "!");
            }
            return true;
          }
          player.sendMessage(ChatColor.DARK_RED + "You can't punish yourself!");
        }
        else {
