        ...
        if ((args.length == 1) && (this.me.getServer().getPlayer(args[0]) != null)) {
          ...
          for (Player p : this.me.getServer().getOnlinePlayers()) {
            if ((p == target) && (p == player)) {
              player.sendMessage(ChatColor.BLUE + "You have freed yourself from torment!");
            }
            else if (p == player) {
              player.sendMessage(ChatColor.BLUE + "You have freed " + target.getName() + " from torment!");
            }
            else if (p == target) {
              target.sendMessage(ChatColor.BLUE + player.getName() + " has freed you from your torment!");
            }
            else
              p.sendMessage(ChatColor.BLUE + player.getName() + " has freed " + target.getName() + " from torment!");
          }
          return true;
        }
