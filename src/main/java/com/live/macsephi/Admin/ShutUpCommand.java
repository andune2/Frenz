    ...
    if ((sender instanceof Player)) {
      ...
      if (this.me.hasPermission(player, "MobEffects.shutup")) {
        if ((args.length == 1) && (this.me.getServer().getPlayer(args[0]) != null)) {
          ...
          if (this.me.hasPermission(target, "MobEffects.shutup.immunity")) {
            player.sendMessage(ChatColor.DARK_RED + target.getName() + " cannot be silenced!");
            return true;
          }
          if (this.me.isMuted.contains(target)) {
            this.me.isMuted.remove(target);
            this.me.getServer().broadcastMessage(ChatColor.GRAY + target.getName() + " has been unmuted!");
            return true;
          }
          this.me.isMuted.add(target);
          this.me.getServer().broadcastMessage(ChatColor.GRAY + target.getName() + " has been muted by " + player.getName() + "!");
          return true;
        }
