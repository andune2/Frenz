...
public class ExtraSpeedCommand
    ...
    if ((sender instanceof Player))
    {
      ...
      if (Frenz.hasPermissions(this.player, "MobEffects.extraspeed"))
      {
        ...
        this.me.getServer().getScheduler().scheduleSyncDelayedTask(this.me, new Runnable()
        {
          public void run()
          {
            if (ExtraSpeedCommand.this.player.hasPotionEffect(PotionEffectType.SPEED))
              ExtraSpeedCommand.this.player.sendMessage(ChatColor.BLUE + "Your energy fades with time, and your speed has returned to normal.");
          }
        }
        , 1150L);
        this.player.sendMessage(ChatColor.BLUE + "You feel a sudden sensation in your legs, as though you can run a marathon without breaking a sweat.");
