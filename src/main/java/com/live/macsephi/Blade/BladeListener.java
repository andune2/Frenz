  ...
  implements Listener
{
  ...
  {
    if (((event.getDamager() instanceof Player)) && (this.me.death.contains(event.getDamager()))) {
    ...
    if ((event.getEntity() instanceof EnderDragonPart)) return;
    int damage;
    int damage;
    if ((event.getDamager() instanceof Player)) {
      Player attacker = (Player)event.getDamager();
      damage = getNewDamage(event.getDamage(), attacker); } else {
    ...
    if (this.me.obsidian.contains(player)) {
      ...
      if (finalDamage > 20) event.setDamage(20); else
  ...
  public int getNewDamage(int damage, Player attacker) {
    if (this.me.bDivine.contains(attacker)) return damage + 50;
    if (this.me.temper.contains(attacker)) return damage + 20;
    if (this.me.extend.contains(attacker)) return damage + 16;
    if (this.me.serrate.contains(attacker)) return damage + 12;
    if (this.me.sharpen.contains(attacker)) return damage + 8;
    if (this.me.carve.contains(attacker)) return damage + 4;
    return damage;
  }
}